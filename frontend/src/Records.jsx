...
export default function Records() {
  ...
  const printBill = (bill) => {
    ...
    if (logo) {
      doc.addImage(logo.src, 'PNG', 10, 10, 50, 15)
    }
    
    // Bill title
    doc.text('Bill Details', 70, 20)
    
    // Bill information
    const billInfo = [
      { label: 'Bill Number', value: `#${bill.code}` },
      { label: 'Vehicle Number', value: bill.vehicle_number },
      { label: 'Date Issued', value: formatDateTime(bill.date_issued) },
      { label: 'Amount', value: `Rs. ${bill.amount?.toLocaleString()}` },
      { label: 'Status', value: bill.status },
      { label: 'Material', value: bill.material },
      { label: 'Issue Location', value: bill.issue_location },
      { label: 'Destination', value: bill.destination },
      { label: 'Region', value: bill.region },
      { label: 'Vehicle Size', value: bill.vehicle_size },
      { label: 'Issued By', value: bill.issued_by_name || 'System' },
      { label: 'Modified By', value: bill.modified_by_name || '-' },
      { label: 'Modified Date', value: bill.modified_date ? formatDateTime(bill.modified_date) : '-' },
      { label: 'Remarks', value: bill.remark || '-' },
    ]
    
    let y = 30
    billInfo.forEach(info => {
      doc.text(`${info.label}: ${info.value}`, 10, y)
      y += 10
    })
    
    // Footer
    doc.setFontSize(10)
    doc.text('Generated by Bill Management System', 10, 280)
  ...
  const exportToPDF = () => {
    try {
      ...
      bills.forEach((bill, index) => {
        ...
        doc.text(`Rs. ${(bill.amount || 0).toLocaleString()}`, 180, yPosition)
        doc.text(bill.status || '', 220, yPosition)
        doc.text(new Date(bill.date_issued).toLocaleDateString(), 250, yPosition)
  ...
  // Print function for the entire table
  const printTable = () => {
    const printContent = document.getElementById('bills-table-container')
    const originalContent = document.body.innerHTML
    
    document.body.innerHTML = `
      <div style="padding: 20px;">
        <h1 style="text-align: center; margin-bottom: 20px;">Bill Records Report</h1>
        ...
        <div style="margin-top: 30px; border-top: 2px solid #333; padding-top: 20px;">
          ...
          <p>Pending Amount: Rs. ${totals.pendingTotal.toLocaleString()}</p>
          <p>Completed Amount: Rs. ${totals.completedTotal.toLocaleString()}</p>
          <p><strong>Total Revenue: Rs. ${totals.grandTotal.toLocaleString()}</strong></p>
          <p style="margin-top: 10px; font-size: 12px; color: #666;">
            Note: Cancelled bills (Rs. ${totals.cancelledTotal.toLocaleString()}) are excluded from revenue calculation
          </p>
        </div>
      </div>
    `
