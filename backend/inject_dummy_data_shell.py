"""
Script to inject dummy data for bills and barcodes via Django shell.
Copy and paste this entire script into the Django shell:

python manage.py shell

Then paste this script and run it.
"""
...
def create_sample_users():
    ...
    for user_data in sample_users:
        ...
        if person_created:
            print(f'Created person for user: {user.email}')

def create_dummy_barcodes(count=20):
    """Create dummy barcodes"""
    persons = list(Person.objects.all())
    if not persons:
        print('No persons found. Creating sample users first...')
        create_sample_users()
        persons = list(Person.objects.all())
    ...
    print(f'Created {created_count} dummy barcodes')

def create_dummy_bills(count=15):
    """Create dummy bills"""
    persons = list(Person.objects.all())
    if not persons:
        print('No persons found. Creating sample users first...')
    ...
    print(f'Created {created_count} dummy bills')

def inject_all_dummy_data(bills_count=15, barcodes_count=20):
    """Inject all dummy data"""
    print("Starting dummy data injection...")
    create_sample_users()
    create_dummy_barcodes(barcodes_count)
    create_dummy_bills(bills_count)
    print("Dummy data injection completed!")
    
    # Print summary
    print(f"\nSummary:")
    print(f"Total Users: {User.objects.count()}")
    print(f"Total Persons: {Person.objects.count()}")
    print(f"Total Bills: {Bill.objects.count()}")
    print(f"Total Barcodes: {Barcode.objects.count()}")

# Run this function to inject dummy data
# inject_all_dummy_data()

print("Script loaded. Run inject_all_dummy_data() to create dummy data.")
print("You can also run individual functions:")
print("- create_sample_users()")
print("- create_dummy_barcodes(20)")
print("- create_dummy_bills(15)")
